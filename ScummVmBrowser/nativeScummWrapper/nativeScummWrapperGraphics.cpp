...
NativeScummWrapper::NativeScummWrapperGraphics::NativeScummWrapperGraphics(f_CopyRect copyRect, f_Blot blot) : GraphicsManager() {
	_copyRect = copyRect;
	_blot = blot;
...
void NativeScummWrapper::NativeScummWrapperGraphics::copyRectToScreen(const void *buf, int pitch, int x, int y, int w, int h) {
	...
	NativeScummWrapper::NativeScummWrapperGraphics::_copyRect(buf, pitch, x, y, w, h, _picturePalette, 255, false, noMessages);

	if (noMessages == 2) {
		if (_cliMouse.width > 0 && _cliMouse.height > 0) {
			NativeScummWrapper::NativeScummWrapperGraphics::_copyRect(_cliMouse.buffer, _cliMouse.fullWidth, _cliMouse.x, _cliMouse.y, _cliMouse.width, _cliMouse.height, _cliMouse.cursorPallette, _cliMouse.keyColor, true, noMessages);
		}
	}
}
...
void NativeScummWrapper::NativeScummWrapperGraphics::warpMouse(int x, int y) {
	if (positionInRange(x, y)) {
		...
		if (shouldBlot) {
			_blot(_cliMouse.prevX, _cliMouse.prevY, _cliMouse.prevW, _cliMouse.prevH, noMessages);
		}
